...
/*
 Reu changes to q5
 - Removed unused vector liberaries
 - Updated image rendering to support masked image rendering
 - Updated canvas creation and resize to support crisp pixel rendering
 - Updated instance management to support scoped namespaces and modules
 - Added additional hooks for missing windowResized method
 - Updated touch and mouse events to watch entire window, not just the canvas
 - Updated canvas context to use gpu caching
*/
...
export function Q5(scope) {
  ...
  function graphics(scope) {
    ...
    Object.defineProperty($, 'deviceOrientation', {
      get() { return Math.abs(window.orientation) == 90 ? $.LANDSCAPE : $.PORTRAIT; },
    });
    ...
    $.createGraphics = $.createImage = function (width, height) {
      const g = new graphics('offscreen');
    ...
    $.map = function (value, istart, istop, ostart, ostop, clamp) {
      const val = ostart + (ostop - ostart) * ((value - istart) * 1.0 / (istop - istart));
      if (!clamp) {
        return val;
      }
      if (ostart < ostop) {
    ...
    function hsv2rgb(h, s, v) {
      ...
      let r; let g; let
        b;
      let hh; let i; let ff; let p; let q; let
        t;
      if (s == 0) {
        r = v;
        g = v;
    ...
    function rgb2hsv(r, g, b) {
      ...
      if (rgbMax == r) { h = 0 + 60 * (g - b) / (rgbMax - rgbMin); } else if (rgbMax == g) { h = 120 + 60 * (b - r) / (rgbMax - rgbMin); } else { h = 240 + 60 * (r - g) / (rgbMax - rgbMin); }
      return [h, s, v];
    }
    ...
    $.stroke = function () {
      ...
      const col = $.color.apply(null, arguments);
      if (col._a <= 0) {
        $._noStroke = true;
        return;
      }
    ...
    function arcImpl(x, y, w, h, start, stop, mode, detail) {
      ...
      const lo = norm2PI(start);
      const hi = norm2PI(stop);
      ctx.beginPath();
    ...
    function catmullRomSpline(p0x, p0y, p1x, p1y, p2x, p2y, p3x, p3y, numPts, alpha) {
      ...
      function catmullromSplineGetT(t, p0x, p0y, p1x, p1y, alpha) {
        const a = (p1x - p0x) ** 2.0 + (p1y - p0y) ** 2.0;
        const b = a ** (alpha * 0.5);
        return (b + t);
      }
      const pts = [];
      ...
      const t0 = 0.0;
      const t1 = catmullromSplineGetT(t0, p0x, p0y, p1x, p1y, alpha);
      const t2 = catmullromSplineGetT(t1, p1x, p1y, p2x, p2y, alpha);
      const t3 = catmullromSplineGetT(t2, p2x, p2y, p3x, p3y, alpha);

      for (let i = 0; i < numPts; i++) {
        const t = t1 + i / (numPts - 1) * (t2 - t1);
        const s = [(t1 - t) / (t1 - t0), (t - t0) / (t1 - t0), (t2 - t) / (t2 - t1), (t - t1) / (t2 - t1),
        ...
        const a1x = p0x * s[0] + p1x * s[1];
        const a1y = p0y * s[0] + p1y * s[1];
        const a2x = p1x * s[2] + p2x * s[3];
        const a2y = p1y * s[2] + p2y * s[3];
        const a3x = p2x * s[4] + p3x * s[5];
        const a3y = p2y * s[4] + p3y * s[5];
        const b1x = a1x * s[6] + a2x * s[7];
        const b1y = a1y * s[6] + a2y * s[7];
        const b2x = a2x * s[8] + a3x * s[9];
        const b2y = a2y * s[8] + a3y * s[9];
        const cx = b1x * s[2] + b2x * s[3];
        const cy = b1y * s[2] + b2y * s[3];
        pts.push([cx, cy]);
      }
      return pts;
    }
    ...
    $.curveVertex = function (x, y) {
      ...
      const p0 = curveBuff[curveBuff.length - 4];
      const p1 = curveBuff[curveBuff.length - 3];
      const p2 = curveBuff[curveBuff.length - 2];
      const p3 = curveBuff[curveBuff.length - 1];
    ...
    $.image = function (img, dx, dy, dWidth, dHeight, sx, sy, sWidth, sHeight) {
      const drawable = img.MAGIC == $.MAGIC ? img.canvas : img;
      function reset() {
    ...
    filterImpl[$.DILATE] = function (data) {
      ...
      for (let i = 0; i < h; i++) {
        for (let j = 0; j < w; j++) {
          const l = 4 * Math.max(j - 1, 0);
          const r = 4 * Math.min(j + 1, w - 1);
          const t = 4 * Math.max(i - 1, 0) * w;
          const b = 4 * Math.min(i + 1, h - 1) * w;
          const oi = 4 * i * w;
          const oj = 4 * j;
          for (let k = 0; k < 4; k++) {
            const kt = k + t;
    ...
    filterImpl[$.BLUR] = function (data, rad) {
      ...
      function gauss1d(ksize) {
        const im = new Float32Array(ksize);
        const sigma = 0.3 * rad + 0.8;
        const ss2 = sigma * sigma * 2;
        for (let i = 0; i < ksize; i++) {
          const x = (i - ksize / 2);
          const z = Math.exp(-(x * x) / (ss2)) / (2.5066282746 * sigma);
          im[i] = z;
        }
        return im;
      }

      const kern = gauss1d(ksize);
      const [w, h] = [ctx.canvas.width, ctx.canvas.height];
      for (let i = 0; i < h; i++) {
        for (let j = 0; j < w; j++) {
          let s0 = 0; let s1 = 0; let s2 = 0; let
            s3 = 0;
          for (let k = 0; k < ksize; k++) {
            const jk = Math.min(Math.max(j - rad + k, 0), w - 1);
            const idx = 4 * (i * w + jk);
      ...
      for (let i = 0; i < h; i++) {
        for (let j = 0; j < w; j++) {
          let s0 = 0; let s1 = 0; let s2 = 0; let
            s3 = 0;
          for (let k = 0; k < ksize; k++) {
            const ik = Math.min(Math.max(i - rad + k, 0), h - 1);
            const idx = 4 * (ik * w + j);
            s0 += tmpBuf[idx] * kern[k];
            s1 += tmpBuf[idx + 1] * kern[k];
            s2 += tmpBuf[idx + 2] * kern[k];
    ...
    function makeTmpBuf() {
      const l = ctx.canvas.width * ctx.canvas.height * 4;
    ...
    $.filter = function (typ, x) {
      const support = $.HARDWARE_FILTERS && (ctx.filter != undefined);
      if (support) {
        makeTmpCtx();
        if (typ == $.THRESHOLD) {
          if (x == undefined) {
    ...
    $.set = function (x, y, c) {
      ...
      const idx = 4 * ((y * $._pixelDensity * ctx.canvas.width) + (x * $._pixelDensity));
      $.pixels[idx] = c._r;
      $.pixels[idx + 1] = c._g;
      $.pixels[idx + 2] = c._b;
    ...
    $.tinted = function () {
      const col = $.color(...Array.from(arguments));
      const alpha = col._a;
      col._a = 1;
      makeTmpCtx();
      tmpCtx.clearRect(0, 0, tmpCtx.canvas.width, tmpCtx.canvas.height);
    ...
    $.loadFont = function (url) {
      const sp = url.split('/');
      const name = sp[sp.length - 1].split('.')[0].replace(' ', '');
    ...
    const scaled_cosine = function (i) { return 0.5 * (1.0 - Math.cos(i * Math.PI)); };
    let p_perlin;
    ...
    $.noise = function (x, y, z) {
      ...
      let xi = Math.floor(x); let yi = Math.floor(y); let
        zi = Math.floor(z);
    ...
    const MULT = (A, B) => [(A)[0] * (B)[0] + (A)[1] * (B)[4] + (A)[2] * (B)[8] + (A)[3] * (B)[12], (A)[0] * (B)[1] + (A)[1] * (B)[5] + (A)[2] * (B)[9] + (A)[3] * (B)[13], (A)[0] * (B)[2] + (A)[1] * (B)[6] + (A)[2] * (B)[10] + (A)[3] * (B)[14], (A)[0] * (B)[3] + (A)[1] * (B)[7] + (A)[2] * (B)[11] + (A)[3] * (B)[15], (A)[4] * (B)[0] + (A)[5] * (B)[4] + (A)[6] * (B)[8] + (A)[7] * (B)[12], (A)[4] * (B)[1] + (A)[5] * (B)[5] + (A)[6] * (B)[9] + (A)[7] * (B)[13], (A)[4] * (B)[2] + (A)[5] * (B)[6] + (A)[6] * (B)[10] + (A)[7] * (B)[14], (A)[4] * (B)[3] + (A)[5] * (B)[7] + (A)[6] * (B)[11] + (A)[7] * (B)[15], (A)[8] * (B)[0] + (A)[9] * (B)[4] + (A)[10] * (B)[8] + (A)[11] * (B)[12], (A)[8] * (B)[1] + (A)[9] * (B)[5] + (A)[10] * (B)[9] + (A)[11] * (B)[13], (A)[8] * (B)[2] + (A)[9] * (B)[6] + (A)[10] * (B)[10] + (A)[11] * (B)[14], (A)[8] * (B)[3] + (A)[9] * (B)[7] + (A)[10] * (B)[11] + (A)[11] * (B)[15], (A)[12] * (B)[0] + (A)[13] * (B)[4] + (A)[14] * (B)[8] + (A)[15] * (B)[12], (A)[12] * (B)[1] + (A)[13] * (B)[5] + (A)[14] * (B)[9] + (A)[15] * (B)[13], (A)[12] * (B)[2] + (A)[13] * (B)[6] + (A)[14] * (B)[10] + (A)[15] * (B)[14], (A)[12] * (B)[3] + (A)[13] * (B)[7] + (A)[14] * (B)[11] + (A)[15] * (B)[15]];
    const TRFM = (A, v) => [((A)[0] * (v)[0] + (A)[1] * (v)[1] + (A)[2] * (v)[2] + (A)[3]) / ((A)[12] * (v)[0] + (A)[13] * (v)[1] + (A)[14] * (v)[2] + (A)[15]), ((A)[4] * (v)[0] + (A)[5] * (v)[1] + (A)[6] * (v)[2] + (A)[7]) / ((A)[12] * (v)[0] + (A)[13] * (v)[1] + (A)[14] * (v)[2] + (A)[15]), ((A)[8] * (v)[0] + (A)[9] * (v)[1] + (A)[10] * (v)[2] + (A)[11]) / ((A)[12] * (v)[0] + (A)[13] * (v)[1] + (A)[14] * (v)[2] + (A)[15])];
